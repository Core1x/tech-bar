// 雷达页「生成 AI 周报」按钮：本周期（截至最新雷达数据日）周报缺失时显示，POST /api/weekly 生成后刷新页面。
// 若已有同一任务在生成（网页/定时任务），后端返回 { running:true }，这里改为轮询状态、完成自动刷新，绝不重复生成。
"use client";
import { useState } from "react";
import { JobRunningNotice } from "@/components/JobRunningNotice";

export function WeeklyGenerateButton({ latestDate }: { latestDate?: string }) {
  ...
  if (queued) {
    return (
      <JobRunningNotice text="该周期的 AI 周报正在生成中（由另一页面或定时任务触发）…完成后会自动刷新" />
    );
  }
  ...
  return (
    <div className="rounded-lg border border-[#f78166]/40 bg-[#f78166]/10 p-4">
      <p className="text-sm text-[#e6edf3]">
        {latestDate ? `截至 ${latestDate} 的 AI 周报尚未生成` : "AI 周报尚未生成"}（每周一篇，幂等生成）
      </p>
